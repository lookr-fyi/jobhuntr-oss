...
app.post('/api/coach/prepare', async (req, res) => {
  const session = await mutate((db) => { const job = db.jobs.find((j) => j.id === req.body.jobId); if (!job) return null; const skills = db.profile.skills || []; const haystack = `${job.title} ${job.description} ${(job.tags||[]).join(' ')}`.toLowerCase(); const relevant = skills.filter((s) => haystack.includes(String(s).toLowerCase())); const matchedStories = db.careerStories.map((story) => ({ story, hits: (story.skills||[]).filter((skill) => haystack.includes(String(skill).toLowerCase())).length })).sort((a,b)=>b.hits-a.hits).filter((x)=>x.hits>0).slice(0,3).map((x)=>x.story.id); const questions = [`Why are you interested in ${job.company} and this ${job.title} role?`, `Describe a project where you used ${relevant[0] || skills[0] || 'your core skills'} to create measurable impact.`, 'Tell me about a difficult tradeoff you made with incomplete information.', `What would your first 30 days at ${job.company} look like?`]; const item = { id: nanoid(), jobId: job.id, status: 'in-progress', createdAt: timestamp(), updatedAt: timestamp(), questions, answers: Object.fromEntries(questions.map((q)=>[q,''])), matchedStoryIds: matchedStories, talkingPoints: [`Connect your headline—“${db.profile.headline}”—to the role.`, `Use a STAR story with a quantified outcome for ${relevant.slice(0,3).join(', ') || 'your most relevant experience'}.`, `Ask how success is measured for the ${job.title} role.`], companyResearch: [`Review the company website and product before the interview.`, `Prepare one informed question about ${job.description || 'the team’s current priorities'}.`], researchDone: [], notes: '' }; db.coachingSessions.unshift(item); auditEvent(db, 'coach', `Prepared local interview plan for ${job.company}.`, { jobId: job.id }); return item; });
  if (!session) return res.status(404).json({ error: 'Job not found' }); res.status(201).json(session);
});
app.patch('/api/coach/sessions/:id', async (req, res) => { const session = await mutate((db) => { const item = db.coachingSessions.find((x) => x.id === req.params.id); if (!item) return null; if (req.body.answers && typeof req.body.answers === 'object') item.answers = Object.fromEntries(item.questions.map((q)=>[q,safeText(req.body.answers[q],10000)])); if (req.body.notes !== undefined) item.notes = safeText(req.body.notes,20000); if (Array.isArray(req.body.researchDone)) item.researchDone = req.body.researchDone.filter((x)=>item.companyResearch.includes(x)); if (['in-progress','completed'].includes(req.body.status)) item.status = req.body.status; item.updatedAt = timestamp(); auditEvent(db, 'coach', `Updated interview practice session.`, { sessionId:item.id }); return item; }); if (!session) return res.status(404).json({ error:'Coaching session not found' }); res.json(session); });

app.get('/api/career-stories', async (_req,res)=>{const db=await readDb();res.json(db.careerStories);});
app.post('/api/career-stories', async (req,res)=>{const story=await mutate((db)=>{const item={id:nanoid(),title:safeText(req.body.title,160)||'Untitled STAR story',situation:safeText(req.body.situation,10000),task:safeText(req.body.task,10000),action:safeText(req.body.action,20000),result:safeText(req.body.result,10000),skills:Array.isArray(req.body.skills)?req.body.skills.map((x)=>safeText(x,100)).filter(Boolean).slice(0,30):[],createdAt:timestamp(),updatedAt:timestamp()};db.careerStories.unshift(item);auditEvent(db,'career-story',`Saved STAR story “${item.title}”.`);return item;});res.status(201).json(story);});
app.patch('/api/career-stories/:id',async(req,res)=>{const story=await mutate((db)=>{const item=db.careerStories.find((x)=>x.id===req.params.id);if(!item)return null;for(const key of ['title','situation','task','action','result'])if(req.body[key]!==undefined)item[key]=safeText(req.body[key],key==='action'?20000:10000);if(Array.isArray(req.body.skills))item.skills=req.body.skills.map((x)=>safeText(x,100)).filter(Boolean).slice(0,30);item.updatedAt=timestamp();return item;});if(!story)return res.status(404).json({error:'Career story not found'});res.json(story);});
app.delete('/api/career-stories/:id',async(req,res)=>{const removed=await mutate((db)=>{const before=db.careerStories.length;db.careerStories=db.careerStories.filter((x)=>x.id!==req.params.id);for(const session of db.coachingSessions)session.matchedStoryIds=(session.matchedStoryIds||[]).filter((id)=>id!==req.params.id);return before!==db.careerStories.length;});res.status(removed?204:404).end();});

app.post('/api/outreach/draft', async (req, res) => {
  const draft = await mutate((db) => { const job = db.jobs.find((j) => j.id === req.body.jobId); if (!job) return null; const contact = job.contacts.find((c) => c.id === req.body.contactId) || job.contacts[0]; const recipient = contact?.name || 'there'; const body = `Hi ${recipient},\n\nI’m exploring the ${job.title} opportunity at ${job.company}. My background in ${(db.profile.skills || []).slice(0,3).join(', ')} looks closely aligned, particularly with ${job.description || 'the team’s product goals'}.\n\nIf you’re open to it, I’d appreciate hearing what the team values most in candidates for this role.\n\nBest,\n${db.profile.name}`; const item = { id: nanoid(), jobId: job.id, contactId: contact?.id || '', channel: req.body.channel === 'email' ? 'email' : 'linkedin', subject: `Interest in ${job.title} at ${job.company}`, body, status: 'draft', createdAt: timestamp(), updatedAt: timestamp() }; db.outreachDrafts.unshift(item); auditEvent(db, 'outreach', `Drafted outreach for ${job.company}.`, { jobId: job.id }); return item; });
  if (!draft) return res.status(404).json({ error: 'Job not found' }); res.status(201).json(draft);
});
app.patch('/api/outreach/:id',async(req,res)=>{const draft=await mutate((db)=>{const item=db.outreachDrafts.find((x)=>x.id===req.params.id);if(!item)return null;if(req.body.subject!==undefined)item.subject=safeText(req.body.subject,300);if(req.body.body!==undefined)item.body=safeText(req.body.body,20000);if(['draft','sent','replied','archived'].includes(req.body.status))item.status=req.body.status;item.updatedAt=timestamp();return item;});if(!draft)return res.status(404).json({error:'Outreach draft not found'});res.json(draft);});
...
app.post('/api/import', async (req, res) => {
  ...
  const allowed = ['meta','profile','jobs','resumes','coverLetters','templates','submissions','coachingSessions','outreachDrafts','huntPresets','careerStories','agentRuns','activities'];
