...
test("document renderer escapes all user-controlled HTML", () => {
  ...
  const resume = renderResumeDocument(
    {
      name: "<img src=x>",
      templateId: "impact",
      content: "SUMMARY\n<script>alert(1)</script>\n- Shipped 20%",
    },
    { name: "Jane <Admin>", headline: "Builder", location: "Remote" },
  );
  assert.doesNotMatch(resume, /<script>alert|<img src=x>/);
  assert.match(resume, /Jane &lt;Admin&gt;/);
  assert.match(resume, /Shipped 20%/);
  const letter = renderCoverLetterDocument(
    { title: "Letter", body: "<iframe>private</iframe>" },
    { name: "Jane" },
